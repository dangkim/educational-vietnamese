...
import { escHTML, shuffle, answersMatch, formatTime, calcStars, starsDisplay } from '../utils/helpers';

// ============================================================
// WORDLE GAME
// ============================================================
export class WordleGame extends BaseGame {
  override gameKey = 'wordle';
  override gameTitle = 'Wordle';
  override gameIcon = '🔤';
  ...
  override setup(): void {
    this.#words = shuffle(store.get('lesson.questions.wordle') ?? []);
    if (!this.#words.length) {
      if (this.body) this.body.innerHTML = `<div class="empty-state"><div class="empty-state-icon">🔤</div><h3>Chưa có từ Wordle</h3></div>`;
      return;
    }
  ...
  get #word(): string { return (this.#words[this.#wordIdx]?.word ?? '').toUpperCase(); }
  get #hint(): string { return this.#words[this.#wordIdx]?.hint ?? ''; }
  get #wordChars(): string[] { return [...this.#word]; }
  ...
  #evaluate(guess: string[], word: string[]): string[] {
  ...
  #renderKeyboard(): void {
    ...
    kb.querySelectorAll('.wordle-key').forEach(btn => {
      btn.addEventListener('click', () => this.#keyPress((btn as HTMLElement).dataset.key!));
    });
  }
  ...
  #attachEvents(): void {
    ...
    this.qs('#w-input')?.addEventListener('keydown', (e: any) => { if (e.key === 'Enter') this.#submitGuess(); });
  }

  #keyPress(k: string): void {
    const input = this.qs('#w-input') as HTMLInputElement;
    if (!input) return;
  ...
  #submitGuess(): void {
    ...
    if ([...guess].length !== len) {
      const status = this.qs('#w-status');
      if (status) status.textContent = `⚠️ Nhập đúng ${len} ký tự!`;
    ...
    if (guess === this.#word) {
      ...
      this.addScore(pts);
      const status = this.qs('#w-status');
      if (status) status.textContent = `🎉 Đúng rồi! +${pts} điểm`;
      ...
      const status = this.qs('#w-status');
      if (status) status.textContent = `😢 Từ đúng là: ${this.#word}`;
...
export class MemoryGame extends BaseGame {
  override gameKey = 'memory';
  override gameTitle = 'Memory Match';
  override gameIcon = '🧩';

  #deck: any[] = [];
  #flipped: number[] = [];  // indices of currently-revealed cards
  #matched = new Set<number>(); // pair IDs
  #moves = 0;
  #locked = false;
  #combo = 0;

  override setup(): void {
    const pairs = store.get('lesson.questions.memory') ?? [];
    if (!pairs.length) {
      if (this.body) this.body.innerHTML = `<div class="empty-state"><div class="empty-state-icon">🧩</div><h3>Chưa có cặp Memory</h3></div>`;
      return;
    }
    // Build deck: each pair → 2 cards
    this.#deck = shuffle(pairs.flatMap((p: any, i: number) => [
  ...
  #renderGrid(): void {
    ...
    grid.querySelectorAll('.mem-card-wrap').forEach(wrap => {
      wrap.addEventListener('click', () => this.#flip(Number((wrap as HTMLElement).dataset.idx)));
    });
  }
  ...
  #flip(idx: number): void {
    ...
    if (this.#flipped.length === 2) {
      ...
      if (this.#deck[a].pairId === this.#deck[b].pairId) {
        ...
        const matchEl = this.qs('#m-match');
        if (matchEl) matchEl.textContent = `${this.#matched.size}/${this.#deck.length / 2}`;
        const comboEl = this.qs('#m-combo');
...
export class FillBlankGame extends BaseGame {
  override gameKey = 'fillblank';
  override gameTitle = 'Điền vào chỗ trống';
  override gameIcon = '✏️';
  ...
  override setup(): void {
    this.#items = store.get('lesson.questions.fillBlank') ?? [];
    if (!this.#items.length) {
      if (this.body) this.body.innerHTML = `<div class="empty-state"><div class="empty-state-icon">✏️</div><h3>Chưa có câu điền trống</h3></div>`;
      return;
    }
  ...
  #renderAll(): void {
    ...
    this.qsAll('.fill-input').forEach(input => {
      input.addEventListener('focus', () => activeInput = input as HTMLInputElement);
    });
    this.qs('#fill-bank')?.addEventListener('click', (e) => {
      const chip = (e.target as HTMLElement).closest('.fill-word-chip') as HTMLElement;
      ...
      if (activeInput) {
        ...
        const inputs = Array.from(this.qsAll('.fill-input')) as HTMLInputElement[];
  ...
  #check(): void {
    ...
    this.#results = this.#items.map((item, i) => {
      const input = this.qs(`#fi-${i}`) as HTMLInputElement;
    ...
    const checkBtn = this.qs('#fill-check') as HTMLButtonElement;
    if (checkBtn) checkBtn.disabled = true;
    setTimeout(() => this.complete(`${correct}/${this.#items.length} câu đúng`), 800);
  }

  override getAnswerData(): any { return { results: this.#results }; }
}
...
export class MCQGame extends BaseGame {
  override gameKey = 'mcq';
  override gameTitle = 'Trắc Nghiệm';
  override gameIcon = '📋';
  ...
  override setup(): void {
    this.#questions = store.get('lesson.questions.multipleChoice') ?? [];
    if (!this.#questions.length) {
      if (this.body) this.body.innerHTML = `<div class="empty-state"><div class="empty-state-icon">📋</div><h3>Chưa có câu trắc nghiệm</h3></div>`;
      return;
    }
  ...
  #renderQuestion(): void {
      ...
      <div class="mcq-stage">
        <div class="mcq-slide">
          ...
          <div class="mcq-options" id="mcq-opts">
            ${this.#q.options.map((opt: string, i: number) => `
    ...
    this.qs('#mcq-opts')?.addEventListener('click', (e) => {
      const btn = (e.target as HTMLElement).closest('.mcq-option') as HTMLElement;
