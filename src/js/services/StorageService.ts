...
export class R2Service {
  ...
  constructor(config: R2Config = {}) { this.#cfg = config; }
  configure(config: R2Config)        { this.#cfg = { ...this.#cfg, ...config }; }
  
  get isConfigured(): boolean { 
    return !!(this.#cfg.accountId && this.#cfg.accessKey && this.#cfg.secretKey && this.#cfg.bucket); 
  }

  async upload(filename: string, data: any, contentType = 'application/json'): Promise<string> {
  ...
  async #hmac(key: string | Uint8Array, msg: string): Promise<Uint8Array> {
    const k = typeof key === 'string' ? new TextEncoder().encode(key) : key;
    const imp = await crypto.subtle.importKey('raw', k as any, { name:'HMAC', hash:'SHA-256' }, false, ['sign']);
    return new Uint8Array(await crypto.subtle.sign('HMAC', imp, new TextEncoder().encode(msg)));
  }
  
  #hex(u8: Uint8Array): string { 
...
export class StorageService {
  static saveLesson(lesson: any) { this.#set(KEYS.LESSON, lesson); }
  static loadLesson()            { return this.#get(KEYS.LESSON); }

  static saveConfig(cfg: any) {
    // Never persist secret key directly — store a flag
    const safe = { ...cfg, r2SecretKey: cfg.r2SecretKey ? '***saved***' : '' };
    this.#set(KEYS.CONFIG, safe);
  }
  static loadConfig() { return this.#get(KEYS.CONFIG); }
  ...
  static clearLesson() { localStorage.removeItem(KEYS.LESSON); }
  static clearAll()    { Object.values(KEYS).forEach(k => localStorage.removeItem(k)); }

  static #set(key: string, value: any) {
    try { localStorage.setItem(key, JSON.stringify({ v: 1, ts: Date.now(), data: value })); }
    catch (e: any) { console.warn('[Storage] Write failed:', e.message); }
  }
