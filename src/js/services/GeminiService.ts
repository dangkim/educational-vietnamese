...
export class GeminiService {
   ...
   */
  async generateQuestions(documentText: string, lessonMeta: { title?: string, subject?: string, grade?: string } = {}): Promise<Questions> {
  ...
  #buildPrompt(text: string, meta: { title?: string, subject?: string, grade?: string }): string {
...
Trả lời CHỈ bằng JSON hợp lệ (KHÔNG dùng markdown, KHÔNG backtick, KHÔNG giải thích):
{
  ...
  async #callWithRetry(prompt: string, attempt = 0): Promise<string> {
    try {
  ...
  #parseResponse(raw: string): Questions {
  ...
  #sanitizeFlashcards(arr: any): any[] {
  ...
  #sanitizeMCQ(arr: any): any[] {
    return (Array.isArray(arr) ? arr : []).filter(i => i?.question && Array.isArray(i.options) && i.options.length >= 2).slice(0, 12).map(i => ({
      ...
      options:     i.options.slice(0, 4).map((o: any) => String(o).trim()),
